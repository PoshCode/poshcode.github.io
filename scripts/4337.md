...
parent:         0
children:       
poster:         Winston McMiller
title:          PS-NTDSUTIL 
date:           2013-07-26 17:16:04
description:    PS-NTDSUTIL 
PowerShell Version of the Classic Active Directory Tool  
The script allows for easy remote or local NTDS operations without using the NTDSUTIL command. Most functions operate without NTDSUTIL except CheckDIT, Offline_Defrag and Create_Full_IFM.
 

Covers Active Directory DS tasks such as: 
•	Moving NTDS.DIT 
•	Moving EDB log 
•	Offline Defragmentation 
•	DS integrity check (Semantic Database consistency check) 
•	Creating IFM Media (AD Snapshots) 
 
Example 1: Move only the NTDS.DIT file 
  PS-NTDSUTIL –DomainController WINDC1 –Move_NTDS_Only

Example 2: Move only the EDB. Log file
   PS-NTDSUTIL –DomainController WINDC1 –Move_DSLog_Only

Example 3: Perform Offline Defragmentation
   PS-NTDSUTIL –DomainController WINDC1 –Offline_Defrag

Example 4: Generates Useful DSA Report of the NTDS configuration
    PS-NTDSUTIL –DomainController WINDC1 –DIT_Report

Example 5: Creates IFM media (AD Snapshot)
   PS-NTDSUTIL –DomainController WINDC1 –Create_FULL_IFM

Example 6: Runs the Semantic Database Analysis
   PS-NTDSUTIL –DomainController WINDC1 –CheckDIT
