...
description:    Two dependent on each other (actually, only Get-Manifest depends on Get-Strings) functions. Get-Manifest tries to get manifest of binary file (for example: PS> mfdump F:\SysInternals\whois.exe); Get-Strings is something that looks like SysInternals strings (PS> strings F:\SysInternals\whois.exe)
format:         posh
---
