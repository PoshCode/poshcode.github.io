...
description:    Manage Priorities of specified apps in a service written in C#. Need to add several references to compile (search web for C# service, it should list them) and needs unsafe code enabled. Early version so the app names are hardcoded, might fix that later. Needs this to install the service: sc create PriorityService binpath=c:\[path of .exe]\Priority-Service.exe start=auto obj=LocalSystem
format:         posh
---
