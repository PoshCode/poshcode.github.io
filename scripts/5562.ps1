...
ktpass /princ "$($PrincipalName)" /mapuser "$($SiteAddress)@$($WinDomain)" /out "C:\$($KeyTab)" /pass "$($PlainTextPassword)" /ptype KRB5_NT_PRINCIPAL
...
<Location /krb>
    ...
    AuthLDAPBindDN $($SiteAddress)@$($WinDomain)
