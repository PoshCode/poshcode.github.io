...
$SecPhrase=ConvertTo-SecureString  $Passphrase Â–AsPlaintext Â–Force
$Passphrase = $null
...
# Create a new farm  
New-SPConfigurationDatabase Â–DatabaseName Â“$FARM-ConfigÂ” Â–DatabaseServer $SQLServer Â–AdministrationContentDatabaseName Â“$FARM-Admin-ContentÂ” Â–Passphrase $SecPhrase Â–FarmCredentials $Cred_Farm
...
If ( $Force ) {
    $Features = Install-SPFeature Â–AllExistingFeatures -force
} else {
    $Features = Install-SPFeature Â–AllExistingFeatures 
}    
