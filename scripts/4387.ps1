function New-Module {
   ...
   #
   #  In one use case, it's just a simplified wrapper for New-ModuleManifest which answers some of the parameters based on the files already in the module directory.
   #
   #  In the second use case, it allows you to collect one or more scripts and put them into a new module directory.
   #.Example
   #  New-Module MyUtility *.ps1 -Author "Joel Bennett" -Description "My collection of utility functions"
   #
   ...
   #
   #  This example shows how to recursively collect all the script files in the directory and it's sub-directories to recursively generate a new module "MyUtility" but leaves the author and description fields at default.
   #.Example
   #  New-Module MyUtility -Upgrade
   #
   #  This example shows how to (re)generate the MyUtility module from all the files that have already been moved to that directory. 
   ...
   param(
      ...
      [ValidateScript({if($_ -match "[$([regex]::Escape(([io.path]::GetInvalidFileNameChars() -join '')))]") { throw "The ModuleName must be a valid directory name. The character '$($matches[0])' is not valid in a Module name."} else { $true } })]
      $ModuleName,
