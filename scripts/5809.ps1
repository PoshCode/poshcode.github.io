...
<#
.Synopsis
   Specify the folder for WinSCP.exe and WinSCP.dll
.DESCRIPTION
   Validates that WinSCP.exe and WinSCP.dll exists, then sets the folder containing WinSCP.exe and WinSCP.dll
.PARAMETER DLLFolder
    String that contains the path containing the WinSCP.dll and WinSCP.dll
.EXAMPLE
   Set-WinSCPDLLFolder -DLLFolder C:\Admin
   Sets the DLL folder for the WinSCPPowershell module to C:\Admin
.NOTES
   The Function will throw an error if the executables do not exist EXACTLY as WinSCP.dll and WinSCP.exe in the target directory.  Once it gets set one time, no cmdlets require it to be provided.
.COMPONENT
   WinSCP.dll
.FUNCTIONALITY
   Sets the folder containing WinSCP.exe and WinSCP.dll
#>
    param(
    ...
    if (test-path $DLLFolder){
        if ((Test-Path "${DLLFolder}\WinSCP.dll")){}
        else{
            throw "no WinSCP.dll found in ${DLLFolder}."
        }
    ...
    add-type -Path "$dllfolder\WinSCP.dll" -ErrorAction stop
    if ($?){
...
.PARAMETER DLLFolder
    The path to the directory containing WinSCP.exe and WinSCP.dll.
.EXAMPLE
...
#>
    ...
    if ($DeleteSourceFilesAfterTransfer){
        $WinSCPSessionObject | New-WinSCPTransfer -TransferType Upload -LocalPath $LocalPath -RemotePath $RemotePath -Mode $Mode -FileMask $FileMask -DeleteSourceFilesAfterTransfer
    }
    else{
        $WinSCPSessionObject | New-WinSCPTransfer -TransferType Upload -LocalPath $LocalPath -RemotePath $RemotePath -Mode $Mode -FileMask $FileMask
    }
...
#>
    ...
    if ($DeleteSourceFilesAfterTransfer){
        $WinSCPSessionObject | New-WinSCPTransfer -TransferType Download -LocalPath $LocalPath -RemotePath $RemotePath -Mode $Mode -FileMask $FileMask -DeleteSourceFilesAfterTransfer
    }
    else{
        $WinSCPSessionObject | New-WinSCPTransfer -TransferType Download -LocalPath $LocalPath -RemotePath $RemotePath -Mode $Mode -FileMask $FileMask
    }
...
if ((Test-Path "$modulePath\WinSCP.exe") -and (Test-Path "$modulePath\WinSCP.dll")){
    Set-WinSCPDLLFolder -DLLFolder $modulePath
}
else{
    Write-host "Did not find WinSCP.exe or WinSCP.dll in the module directory $script:WinSCPDLLFolder. Specifying -DLLFolder will be required before using this module" -ForegroundColor DarkYellow
}
