...
description:    Audit script by Alan Renouf, Usage: Audit.ps1 'path' path needs to be in single quotes.  The file needs to be a plain text list of computers to be audited one on each line.  Output will be a htm file for each server with detailed system information. Any comments please contact me.
format:         posh
---
