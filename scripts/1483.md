...
description:    Another way to download all the session videos via BITS transfer (no curl needed) with PowerShell 2.0 (This version creates separate jobs for each file, so you have to be LOGGED IN as an admin, and in an ELEVATED host to use this method, because otherwise you hit the limit on the number of bits jobs).
format:         posh
---
