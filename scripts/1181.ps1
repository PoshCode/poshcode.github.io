...
## # Parameter name (and command alias) expansion with Ctrl+E
##  ls -f *.txt -r | ? { $_.LastWriteTime -lt "2009/5" } | Select-String powershell -e OEM<Ctrl+E><tab>
##
##  Get-ChildItem -Filter *.txt -Recurse | Where-Object { $_.LastWriteTime -lt "2009/5" } | Select-String powershell -Exclude OEM<tab>
##  Get-ChildItem -Filter *.txt -Recurse | Where-Object { $_.LastWriteTime -lt "2009/5" } | Select-String powershell -Encoding OEM<tab>
##  Get-ChildItem -Filter *.txt -Recurse | Where-Object { $_.LastWriteTime -lt "2009/5" } | Select-String powershell -ErrorAction OEM<tab><tab>...
...
function TabExpansion {
            ...
            & {
                ...
                switch ([int]$line[-1])
                {
                    ...
                    # Ctrl+E command alias and parameter name expansion
                    5 {
                        if ( $line -ne $( "=" + [Char]5 ) )
                        {
                            ...
                            [Management.Automation.PSParser]::Tokenize($line, [ref]$null) |
                             % {
                                $word = $_.Content
                                $head = $_.Start
                                $spaces = ' ' * ($head-$tail)
                                ...
                                {
                                    "Command" {
                                        $global:ctrlE = $global:ctrlE | % { $_ + $spaces + ($cmdinf = gcm ($word -replace '\?','`?')).Definition.Split(" ")[0] }
                                    }

                                    "CommandParameter" {
                                       $_pat = $word.TrimStart("-") + "*"
                                       $global:ctrlE = $global:ctrlE | % { $temp = $_; $cmdinf.Parameters.Keys -like $_pat | % { $temp + $spaces + "-" + $_ } }
                                    }
                                    ...
                                    "Variable" {
                                        if ( $line[$head] -eq '$' )
                                        {
                                            ...
                                            if ( $line[$head+1] -eq '{' )
                                            {
                                                $global:ctrlE = $global:ctrlE | % { $_ + $spaces + '${' + $word + '}' }
                                                continue;
                                            }
                                        }
                                        elseif ( $line[$head] -eq '@' )
                                        ...
                                        $global:ctrlE = $global:ctrlE | % { $_ + $spaces + $sigil + $word }
                                    }
                                    ...
                                    "String" {
                                        ...
                                        $global:ctrlE = $global:ctrlE | % { $_ + $spaces + $quot + $word + $quot }
                                    }
                            ...
                            [Windows.Forms.SendKeys]::SendWait("{Esc}=^E{TAB}")
                        }
                        else
                        {
                            $global:ctrlE;
