...
.DESCRIPTION
This script exports a range (ex. A5:L20) in a worksheet (ex. Sheet1) in a workbook (ex. c:\temp\myfile.xlsx) to a PNG file by using the clipboard.
...
This script will probably need some tuning on other languages and/or powershell versions. Tested with PS 3.0 and Excel 2010 on Win 7 64bit EN-US.

If Excel is in another culture than powershell, you will most likely run into problems. A typical error is the "Old format or invalid type library" - see http://support.microsoft.com/kb/320369
...
By default, the script do not force-close Excel before starting to parse. This parameter does just that to ensure that the COM object gets a clean start.
...
if (-not $donotforceexcelclose) { Get-Process EXCEL | Stop-Process -Force }
    ...
    $_.sheet | % {
        ...
        $_.range | % {
            ...
            $rng=$sh.range($range)
             ...
             # Ref http://msdn.microsoft.com/en-us/library/microsoft.office.interop.excel.range.copypicture(v=office.15).aspx
...
$xl.Quit()
if (([System.Runtime.Interopservices.Marshal]::ReleaseComObject($xl))) {  Write-Error "Couldn't release COM object. Please kill excel process manually" }
} catch {
