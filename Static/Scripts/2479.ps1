...
process {
   ...
   if($WithArgs) {
      ...
      $Type.GetMethod($MethodName, $ArgumentTypes).MakeGenericMethod($Types).Invoke( $on, $Arguments )
   } else {
      [Type[]]$ArgumentTypes = New-Object Type[] 0
      $Type.GetMethod($MethodName, $ArgumentTypes).MakeGenericMethod($Types).Invoke( $on )
   }
...
$SnapinTypes = Get-Assembly $Snapin | Get-Type -BaseType System.Management.Automation.PSSnapIn -WarningAction SilentlyContinue
...
foreach($SnapinType in $SnapinTypes) {
   ...
   $Types = $Installer.Types | Resolve-Path -ErrorAction Continue
   ...
   New-ModuleManifest -Path $ManifestPath -Author $Author -Company $Installer.Vendor -Description $Installer.Description `
                      -ModuleToProcess $SnapinType.Assembly.Location -Types $Types -Formats $Formats `
...
Set-Alias gax Get-Accelerator
Set-Alias aax Add-Accelerator
Set-Alias rax Remove-Accelerator
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUkJYW6JPKxz+4dOokg9yawy4i
# J3ugggUrMIIFJzCCBA+gAwIBAgIQKQm90jYWUDdv7EgFkuELajANBgkqhkiG9w0B
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUT+azELP/
# woY6L3YYUM4jLhkFEw4wDQYJKoZIhvcNAQEBBQAEggEALhaeyR7d+6dPmocW6XV0
# Ik0c+u4rAS0aQyB9QWYcBwDSqCRRdQx6J4UVC13Lf3Dk2PEo3QzoiTYxvdlkZT4Q
# q67JgT7OW5XKjiOKMQYh/zlydc7To2/vtCiu4/vZDEU2sMwlhu8E7LHuTRMODt9j
# OnmQZus1asmkwt4cIvJK31FoZfdBHxUMgUwBsx3BBn8O4iGq4GfZHGrxsU7an/bf
# +KANyJ3y4LnolTHx+74/9rAO83HJY/6Ps1/VwBEm/bXYmley806QyQwyK2kM7gya
# QACvIz6ZOELUmZA52y0fnJ+5NRJovYhblSqoY1qm4jkU9nC3AImF3BunR74YZULZ
# lQ==
