...
Now hashtable variable and '.<tab>' show its key name
Get-Process | Get-Unique | % { $hash += @{$_.ProcessName=$_} }
$hash.pow<tab>.pro<tab>

Better '$_.' expansion with 2 times or more 'Select-Object -ExpandProperty' combination expressions
gcm Add-Content |select -exp Par<tab>|select -exp <tab> |
 select -ExpandProperty Par<tab>| | ? { $_.Par<tab>.N<tab> -eq "string" }
*this require latest Get-PipeLineObject.ps1 in same directory 
